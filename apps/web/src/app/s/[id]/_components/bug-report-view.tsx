      ...
      {input.data.canEdit ? (
        <EditBugReportSheet
          ...
          report={{
            ...
            roomId: input.data.room?.id ?? null,
            roomName: input.data.room?.name ?? null,
            roomColor: input.data.room?.color ?? null,
            testedFeature: input.data.testedFeature,
            testScenario: input.data.testScenario,
            testCaseType: input.data.testCaseType,
          }}
        />
      ) : null}
...
export function BugReportView({ id }: BugReportViewProps) {
  ...
  const networkRequests = useMemo(() => {
    const pagedRequests =
      networkRequestsQuery.data?.pages.flatMap((page) => page.items) ?? []

    // Pages are fetched at different moments, so an overlap is always possible
    // (a cached page from before a sort fix, or rows shifting between fetches).
    // Collapsing by id keeps React keys unique no matter what the pages hold.
    return Array.from(
      new Map(pagedRequests.map((request) => [request.id, request])).values()
    )
