...
/** Select value that clears the room on every selected report. */
export const BULK_NO_ROOM_VALUE = "__none__"
...
interface BugReportsBulkActionsProps {
  ...
  onBulkRoomChange: (value: string) => void
...
export function BugReportsBulkActions({
  ...
  const roomLabel = (() => {
    ...
    if (bulkRoom === BULK_NO_ROOM_VALUE) {
      return "Remove from room"
    }

    return visibleRooms.find((room) => room.id === bulkRoom)?.name ?? "Room"
  })()
      ...
      <Dialog onOpenChange={setIsEditorOpen} open={isEditorOpen}>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Select
              onValueChange={(value) => onBulkRoomChange(value as string)}
              value={bulkRoom}
            >
              <SelectTrigger className="w-full sm:col-span-2">
              ...
              <SelectContent>
                <SelectItem value={BULK_NO_ROOM_VALUE}>
                  Remove from room
                </SelectItem>
