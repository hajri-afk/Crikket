...
export const bugReport = pgTable(
  "bug_report",
  {
    ...
    // QA testing scope: which feature/menu was exercised, the steps taken, and
    // whether the checks were positive, negative, or both.
    testedFeature: text("tested_feature"),
    testScenario: text("test_scenario"),
    testCaseType: text("test_case_type"), // positive | negative | both
  ...
  (table) => [
    ...
    index("bug_report_roomId_idx").on(table.roomId),
...
export const bugReportUploadSession = pgTable(
  "bug_report_upload_session",
  {
    ...
    testedFeature: text("tested_feature"),
    testScenario: text("test_scenario"),
    testCaseType: text("test_case_type"),
