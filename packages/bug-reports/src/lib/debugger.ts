...
> {
  const networkRequests = await db
    ...
    // Requests fired in the same millisecond are common, and `timestamp` alone
    // is not a stable sort key: with LIMIT/OFFSET the same row could land on two
    // pages (duplicate React keys) while another row was skipped entirely.
    .orderBy(
...
export async function getBugReportDebuggerEventsData(
  ...
  const [actions, logs] = await Promise.all([
    db
      ...
      .where(eq(bugReportAction.bugReportId, bugReportId))
      .orderBy(asc(bugReportAction.timestamp), asc(bugReportAction.id)),
    db
      .select({
        id: bugReportLog.id,
      ...
      .orderBy(asc(bugReportLog.timestamp), asc(bugReportLog.id)),
  ])
