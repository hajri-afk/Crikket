...
import { assertRoomInOrganization } from "./rooms"
...
export const createBugReportUploadSessionInputSchema = z.object({
  ...
  testedFeature: optionalText(TESTED_FEATURE_MAX_LENGTH),
  testScenario: optionalText(TEST_SCENARIO_MAX_LENGTH),
  testCaseType: z.enum(TEST_CASE_TYPE_VALUES).optional(),
...
export async function createBugReportUploadSession(input: {
  ...
  // A capture can only land in a room owned by the same organization, and
  // archived rooms stop accepting new recordings.
  ...
  const result = await retryOnUniqueViolation(async () => {
    ...
    await db.insert(bugReportUploadSession).values({
      ...
      testedFeature: input.input.testedFeature,
      testScenario: input.input.testScenario,
      testCaseType: input.input.testCaseType,
...
export async function finalizeBugReportUpload(input: {
  ...
  await db.transaction(async (tx) => {
    await tx.insert(bugReport).values({
      ...
      testedFeature: uploadSession.testedFeature,
      testScenario: uploadSession.testScenario,
      testCaseType: uploadSession.testCaseType,
