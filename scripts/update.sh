...
update_checkout() {
  if ! command_exists git; then
    warn "git is not installed. Skipping source update check."
    return 0
  fi

  if ! git -C "$ROOT_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    warn "This directory is not a git checkout. Skipping git pull."
    return 0
  fi
  ...
  if ! git -C "$ROOT_DIR" rev-parse --abbrev-ref --symbolic-full-name "@{u}" >/dev/null 2>&1; then
    warn "Current branch has no upstream configured. Skipping git pull."
    return 0
  fi
  ...
  if [[ "$behind" == "0" ]]; then
    info "Repo checkout is already up to date."
    return 0
  fi

  if [[ "$ahead" != "0" ]]; then
    warn "Local branch has diverged from upstream. Skipping git pull."
    return 0
  fi
